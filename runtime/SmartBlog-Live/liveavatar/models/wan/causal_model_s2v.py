...
def _as_python_int(v, default: int = 0) -> int:
    try:
        return int(v)
    except Exception:
...
def _write_streaming_kv_cache(cache_tensor, current_start: int, values):
    cache_size = max(0, int(cache_tensor.shape[1]))
    current_start = max(0, int(current_start))
    seg_len = max(0, int(values.shape[1]))
    if cache_size == 0 or seg_len == 0:
        return 0, 0, 0

    active_before = min(current_start, cache_size)
    if current_start + seg_len <= cache_size:
        end = current_start + seg_len
        cache_tensor[:, current_start:end] = values
        return int(current_start), int(end), 0

    write_len = min(seg_len, cache_size)
    keep_len = min(active_before, cache_size - write_len)
    keep_src_start = active_before - keep_len
    if keep_len > 0:
        keep_src_end = keep_src_start + keep_len
        cache_tensor[:, :keep_len] = cache_tensor[:, keep_src_start:keep_src_end].clone()

    write_src_start = seg_len - write_len
    write_dst_start = keep_len
    write_dst_end = write_dst_start + write_len
    cache_tensor[:, write_dst_start:write_dst_end] = values[:, write_src_start:write_src_start + write_len]
    overflow = max(0, current_start + seg_len - cache_size)
    return int(write_dst_start), int(write_dst_end), int(overflow)
...
def sp_attn_forward_s2v(self,
        ...
        if kv_cache is None:
            ...
            if global_seg_idx[1]-global_seg_idx[0] > 0: #streaming inference
                values_k = k[:,global_seg_idx[0]:global_seg_idx[1]]
                values_v = v[:,global_seg_idx[0]:global_seg_idx[1]]
                write_start = int(current_start)
                current_start, active_kv_cache_size, _ = _write_streaming_kv_cache(
                    kv_cache["k"][:, :, head_start_rank:head_end_rank],
                    current_start=write_start,
                    values=values_k,
                )
                _write_streaming_kv_cache(
                    kv_cache["v"][:, :, head_start_rank:head_end_rank],
                    current_start=write_start,
                    values=values_v,
                )
                k_lens_val = int(active_kv_cache_size + int(kv_cache["cond_end"]))
                x = attention(
                    q=q[:,global_seg_idx[0]:global_seg_idx[1]],
                    k=torch.cat(
                                [
                                kv_cache["k"][:, :active_kv_cache_size, head_start_rank:head_end_rank],
                                kv_cache["cond_k"][:, :int(kv_cache["cond_end"]), head_start_rank:head_end_rank]
                                ],dim=1
                                ),
                    v=torch.cat(
                                [
                                kv_cache["v"][:, :active_kv_cache_size, head_start_rank:head_end_rank],
                                kv_cache["cond_v"][:, :int(kv_cache["cond_end"]), head_start_rank:head_end_rank]
                                ],dim=1
                                ),
                    k_lens=torch.full((int(b),), int(k_lens_val), dtype=torch.long, device=q.device),
                ...
                x = attention(
                    ...
                    k_lens=torch.full(
                        (int(b),),
                        int(global_seg_idx[2] - global_seg_idx[1]),
                        dtype=torch.long,
                        device=q.device,
                    ),
                    window_size=self.window_size)
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, block_mask, kv_cache=None, current_start=0, current_end=0, sp_size=None,seg_idx=None,freqs_cond=None):
        ...
        if kv_cache is None:
            ...
            if seg_idx[1]-seg_idx[0] > 0: #streaming inference
                ...
                values_k = roped_key[:, seg_idx[0]:seg_idx[1]]
                values_v = v[:, seg_idx[0]:seg_idx[1]]
                write_start = int(current_start)
                _, active_kv_cache_size, _ = _write_streaming_kv_cache(
                    kv_cache["k"],
                    current_start=write_start,
                    values=values_k,
                )
                ...
                if self.local_attn_size != -1:
                    ...
                    active_kv_cache_start = max(
                        0,
                        int(active_kv_cache_size) - self.local_attn_size * int(seg_len_block) // 3,
                    )
                k_lens_val = int(active_kv_cache_size - active_kv_cache_start + active_cond_cache_size)
                x = attention(
                ...
                cond_end = int(kv_cache["cond_end"])
                x = attention(
                    q=roped_query[:,seg_idx[1]:seg_idx[2]],
                    k=causal_rope_apply_cond(
                            k, grid_sizes, freqs
                        ).type_as(v)[:,:cond_end],
                    v=kv_cache["cond_v"][:, :cond_end],
                    k_lens=torch.full((int(b),), int(cond_end), dtype=torch.long, device=q.device),
                    window_size=self.window_size
                )
            else:
                assert False, "segment index is invalid"
    ...
    def forward(self, x, e, seq_lens, grid_sizes, freqs, context, context_lens,
        ...
        for element in e: #element: [B,F,2,dim]
            ...
            if torch.is_tensor(seq_lens):
                seq_len_value = seq_lens[0] if seq_lens.dim() > 0 else seq_lens
            else:
                seq_len_value = seq_lens
            cond_len = max(0, _as_python_int(seq_len_value, 0) - int(element_noisy.shape[1]))
            element_cond = element[:,0:1,1].repeat(1,cond_len,1) #[B,cond_len,dim]
        ...
        x = self._cross_attn_ffn(x, context, context_lens, e, bf_dtype_tensor)
        return x

    @conditional_compile
    def _cross_attn_ffn(self, x, context, context_lens, e, bf_dtype_tensor):
        ...
        with amp.autocast(dtype=torch.float32):
            y = y * e[5]
            x = x + y
        return x.type_as(bf_dtype_tensor)
...
class CausalWanModel_S2V(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.freqs = torch.cat([
            ...
            rope_params(45000, 2 * (d // 6))
                               ...
                               dim=1).to("cuda")
        self.rope_cache = {}
        self._stream_rope_precompute_cache = {}
        self._stream_static_precompute_cache = {}
        self._stream_rope_cond_grid_key = None
    ...
    def inject_motion(self,
        ...
        if len(mot) > 0:
            ...
            base_shape = tuple(int(v) for v in self.rope_cache['cond_shape'])
            motion_shape = tuple(int(v) for v in motion_rope_cache['cond_shape'])
            if len(base_shape) == 4 and len(motion_shape) == 4:
                self.rope_cache['cond_shape'] = torch.Size((
                    base_shape[0],
                    base_shape[1] + motion_shape[1],
                    base_shape[2],
                    base_shape[3],
                ))
            else:
                self.rope_cache['cond_shape'] = torch.cat(
                                                    ...
                                                    dim=1).shape
            motion_grid_sizes = self._stream_repeat_grid_batch(
                motion_rope_cache['grid_sizes'],
                int(base_shape[0]) if len(base_shape) > 0 else len(x),
            )
            self.rope_cache['grid_sizes'] = self.rope_cache['grid_sizes'] + motion_grid_sizes
            self._stream_rope_cond_grid_key = self._stream_rope_grid_key(
                self.rope_cache.get("grid_sizes", None)
            )
            mask_input = [
                torch.cat([
    ...
    @staticmethod
    def _stream_repeat_grid_batch(grid_sizes, batch_size: int):
        batch_size = max(1, int(batch_size or 1))

        def _repeat(item):
            if torch.is_tensor(item):
                if item.dim() >= 2 and int(item.shape[0]) == 1 and batch_size > 1:
                    return item.repeat(batch_size, *([1] * (item.dim() - 1)))
                return item
            if isinstance(item, list):
                return [_repeat(v) for v in item]
            if isinstance(item, tuple):
                return tuple(_repeat(v) for v in item)
            return item

        return _repeat(grid_sizes)

    @staticmethod
    def _stream_rope_grid_key(grid_sizes):
        def _item_key(item):
            if torch.is_tensor(item):
                if item.numel() == 0:
                    return ()
                return tuple(int(v) for v in item.detach().reshape(-1).cpu().tolist())
        ...
        return _item_key(grid_sizes)

    @staticmethod
    def _stream_tensor_key(tensor):
        ...
        if isinstance(tensor, (list, tuple)):
            return tuple(CausalWanModel_S2V._stream_tensor_key(v) for v in tensor)
        return (type(tensor).__name__, int(id(tensor)))

    def _stream_static_cache_get_or_set(self, key, compute_fn):
        if not _env_flag("LIVE_STREAM_STATIC_PRECOMPUTE_CACHE", True):
            return compute_fn()
        try:
            cache = getattr(self, "_stream_static_precompute_cache", None)
            if not isinstance(cache, dict):
                cache = {}
                self._stream_static_precompute_cache = cache
            cached = cache.get(key)
            if cached is not None:
                return cached
            value = compute_fn()
            max_entries = max(4, min(128, _env_int("LIVE_STREAM_STATIC_PRECOMPUTE_CACHE_MAX", 32)))
    ...
    def _cached_stream_audio_embeddings(self, audio_input, motion_frames, current_start: int):
        motion_key = tuple(int(v) for v in motion_frames)
        key = (
            "audio",
            int(current_start),
            motion_key,
            str(getattr(self, "dtype", "")),
            bool(getattr(self, "enbale_adain", False)),
            self._stream_tensor_key(audio_input),
        )

        def _compute():
            expanded_audio = torch.cat([
                audio_input[..., 0:1].repeat(1, 1, 1, motion_frames[0]), audio_input
            ], dim=-1)
            audio_emb_res = self.casual_audio_encoder(expanded_audio)
            return tuple(aa.to(self.dtype) for aa in audio_emb_res)

        return self._stream_static_cache_get_or_set(key, _compute)

    def _cached_stream_cond_embeddings(self, cond_states, current_start: int):
        key = (
            "cond",
            int(current_start),
            self._stream_tensor_key(cond_states),
            str(self.cond_encoder.weight.device) if hasattr(self, "cond_encoder") else "",
            str(self.cond_encoder.weight.dtype) if hasattr(self, "cond_encoder") else "",
        )

        def _compute():
            return tuple(self.cond_encoder(c.unsqueeze(0)) for c in cond_states)

        return self._stream_static_cache_get_or_set(key, _compute)

    def _cached_stream_context_embedding(self, context, current_start: int):
        key = (
            "context",
            int(current_start),
            self._stream_tensor_key(context),
            str(self.text_embedding[0].weight.device),
            str(self.text_embedding[0].weight.dtype),
        )
    ...
    def _cached_stream_rope_precompute(
            self,
            x,
            grid_sizes,
            freqs,
            *,
            cache_name: str,
            extra_key=()):
        if (not _env_flag("LIVE_ROPE_PRECOMPUTE_CACHE", True)) or bool(getattr(self, "use_context_parallel", False)):
            return rope_precompute(x, grid_sizes, freqs, start=None)

        try:
            cache = getattr(self, "_stream_rope_precompute_cache", None)
            if not isinstance(cache, dict):
                cache = {}
                self._stream_rope_precompute_cache = cache
            device_idx = int(x.device.index or 0) if x.device.type == "cuda" else -1
            key = (
                str(cache_name),
                int(device_idx),
                str(x.dtype),
                tuple(int(v) for v in x.shape),
                tuple(extra_key) if isinstance(extra_key, (list, tuple)) else (extra_key,),
            )
            cached = cache.get(key)
            if torch.is_tensor(cached) and cached.device == x.device:
                return cached

            value = rope_precompute(x, grid_sizes, freqs, start=None)
            max_entries = max(4, min(64, _env_int("LIVE_ROPE_PRECOMPUTE_CACHE_MAX", 16)))
            if len(cache) >= int(max_entries):
                cache.clear()
            cache[key] = value
            return value
    ...
    def after_transformer_block(self, block_idx, hidden_states, mask=None):
        if block_idx in self.audio_injector.injected_block_id.keys():
            ...
            elif num_actors > 1 and int(hidden_states.shape[0]) != int(num_actors):
    ...
    def _forward_sink(
        ...
        self._stream_rope_cond_grid_key = self._stream_rope_grid_key(grid_sizes)
        rollout_frames = _as_python_int(current_start // frame_seqlen, 0)
        self.pre_compute_freqs = rope_precompute(
            ...
            x.detach().view(b, s, n, d), rollout_grid_sizes(grid_sizes, rollout_frames), self.freqs, start=None )
    ...
    def _forward_inference(
        ...
        audio_emb_res = self._cached_stream_audio_embeddings(
            audio_input,
            motion_frames,
            current_start=_as_python_int(current_start, 0),
        ) # tuple(2),[0]:torch.Size([1, 39, 1, 5120]),[1]:torch.Size([1, 39, 5, 5120])
        if self.enbale_adain:
        ...
        # cond states
        cond = self._cached_stream_cond_embeddings(
            cond_states,
            current_start=_as_python_int(current_start, 0),
        )#cond[0].shape:torch.Size([1, 5120, 20, 24, 16])
        x = [x_ + pose for x_, pose in zip(x, cond)] #list(bs):torch.Size([1, 5120, 20, 24, 16])
        stream_rope_grid_key = tuple(tuple(int(v) for v in u.shape[2:]) for u in x)

        grid_sizes = torch.stack(
            [torch.tensor(u.shape[2:], dtype=torch.long, device=device) for u in x]) #tensor([[20, 24, 16]])
        x = [u.flatten(2).transpose(1, 2) for u in x] # list , torch.Size([1, 7680, 5120])
        ...
        rollout_frames = _as_python_int(current_start // frame_seqlen, 0)
        main_rope_grid = rollout_grid_sizes(grid_sizes, rollout_frames)
        self.pre_compute_freqs = self._cached_stream_rope_precompute( #可以做一点加速这里大概0.06秒，其中0.45秒是计算（cpu串行可以挪到gpu），分配内存0.15秒可以cache
            x.detach().view(b, s, n, d),
            main_rope_grid,
            self.freqs,
            cache_name="main",
            extra_key=(stream_rope_grid_key, int(rollout_frames)),
        )

        # NOTE: The original code used a per-call random relative_dist, which makes
        # rotary conditioning inconsistent across diffusion steps and can cause
        # "scaly/mosaic" artifacts as you increase sampling_steps.
        #
        # Keep it deterministic (tunable via env) so quality scales with steps as
        # expected. In practice, very large distances (e.g. 30) can make the
        # reference conditioning "too far" in RoPE index space for long rollouts,
        # so we default to a mid value.
        try:
            relative_dist = int(os.getenv("LIVE_ROPE_RELATIVE_DIST", "12") or 12)
        except Exception:
            relative_dist = 12
        relative_dist = max(4, min(30, int(relative_dist)))
        start_idx = 30 - relative_dist
        num_frames_cond_rollout = max(0, _as_python_int(current_start // frame_seqlen, 0) - start_idx)
        # Use KV-cache tensors as the shape source for cond rotary freqs. This is more robust under
        # torch.compile/fake tensor tracing than relying on a cached Python-side shape.
        try:
            if isinstance(kv_cache, (list, tuple)) and len(kv_cache) > 0 and isinstance(kv_cache[0], dict):
                cond_len = int(kv_cache[0].get("cond_end", 0) or 0)
                cond_src = kv_cache[0].get("cond_k", None)
            elif isinstance(kv_cache, dict):
                cond_len = int(kv_cache.get("cond_end", 0) or 0)
                cond_src = kv_cache.get("cond_k", None)
            else:
                cond_len = 0
                cond_src = None
        except Exception:
            cond_len = 0
            cond_src = None
        cond_len = max(0, int(cond_len))
        if cond_src is None:
            cond_src = torch.empty(
                (x.size(0), 0, self.num_heads, self.dim // self.num_heads),
                device=x.device,
                dtype=x.dtype,
            )
        else:
            cond_src = cond_src[:, :cond_len]

        cond_rope_grid = rollout_grid_sizes(self.rope_cache["grid_sizes"], num_frames_cond_rollout)
        cond_pre_compute_freqs = self._cached_stream_rope_precompute(
            cond_src.type_as(x),
            cond_rope_grid,
            self.freqs,
            cache_name="cond",
            extra_key=(
                getattr(self, "_stream_rope_cond_grid_key", None),
                int(num_frames_cond_rollout),
                int(cond_len),
            ),
        )
        ...
        # context
        context_lens = None
        context = self._cached_stream_context_embedding(
            context,
            current_start=_as_python_int(current_start, 0),
        )

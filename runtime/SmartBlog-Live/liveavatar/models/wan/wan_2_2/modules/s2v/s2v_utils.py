...
# @conditional_compile
def _cyclic_frame_indices(indices, freq_len):
    if not indices:
        return indices
    freq_len = int(freq_len)
    if freq_len <= 0:
        return indices
    if min(indices) >= 0 and max(indices) < freq_len:
        return indices
    return [int(idx) % freq_len for idx in indices]
        ...
        for i in range(batch_size):
            ...
            if seq_len > 0:
                if t_f > 0:
                    ...
                    f_sam = _cyclic_frame_indices(f_sam, freqs[0].shape[0])
